package org.example.streampractice.practice;

import org.example.streampractice.model.*;
import org.example.streampractice.service.*;
import org.example.streampractice.model.*;
import org.example.streampractice.service.*;
...
//        return employeeService.readAll().stream()
//                .max(Comparator.comparing(Employee::getSalary))
//                .orElseThrow(() -> new Exception("Not Found.")).getJob();
    ...
    public static Long getMaxSalaryInAmericasRegion() throws Exception {
        ...
        return employeeService.readAll().stream()
                .filter(employee -> employee.getDepartment().getLocation().getCountry()
...
//                .collect(Collectors.toList());
    ...
    public static Employee getEmployeeOfJobHistoryWhoseStartDateIsFirstDayOfJanuary2007AndEndDateIsLastDayOfDecember2007AndDepartmentNameIsShipping() throws Exception {
        ...
        // if we don't use "map" operation in the above stream, we must use getEmployee() by additionally
    ...
    public static List<Employee> getLongestNamedEmployee() {
        return employeeService.readAll().stream()
                .filter(employee -> employee.getFirstName().length() + employee.getLastName().length() + 1
